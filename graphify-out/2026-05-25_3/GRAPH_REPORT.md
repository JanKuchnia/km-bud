...
## Corpus Check
- 206 files · ~1,563,547 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4405 nodes · 4458 edges · 401 communities (391 shown, 10 thin omitted)
...
Cohesion: 0.08
Nodes (24): Anti-Patterns (AVOID), Brainstorming & Communication Protocol, code:block1 (0. CHECK MEMORY — Does .agent/memory/MEMORY.md exist?), code:block2 (1. Parse request → Extract domain, features, scale indicator), code:markdown (### [PRIORITY] **[DECISION POINT]**), code:block4 (1. Acknowledge the error), code:block5 (1. Success confirmation (celebrate briefly)), Communication Principles (+16 more)
...
Cohesion: 0.09
Nodes (21): AG Kit Skills, code:block1 (my-skill/), code:bash (mkdir -p .agent/skills/code-review), code:markdown (---), code:python (import time), code:bash (mkdir -p .agent/skills/license-header-adder/resources), code:block6 (/*), code:markdown (---) (+13 more)
...
Cohesion: 0.20
Nodes (10): code:block10 (1. explorer-agent → Map affected areas), code:markdown (## Orchestration Report), code:bash (# 1. Check for task plan), code:block9 (What domains does this task touch?), Orchestration Workflow, 🔴 STEP 0: PRE-FLIGHT CHECKS (MANDATORY), Step 1: Task Analysis, Step 2: Agent Selection (+2 more)
...
Cohesion: 0.22
Nodes (9): code:block12 (User Request → DECOMPOSE → CLASSIFY → DISPATCH → MONITOR → S), code:block13 (❌ WRONG: "Based on your findings, fix the bug"), Context Compression, Coordinator Lifecycle, 🚀 Coordinator Mode (2026.5.13), Fork Semantics, Memory Integration, Phase-Based Workflow (+1 more)
...
Cohesion: 0.40
Nodes (5): code:block14 (🔴 STEP 0: Pre-flight Check), code:block15 (❌ SKIP Step 0 check), code:block16 (🔴 STEP 0: Pre-flight Check), Example Orchestration, ❌ WRONG Example (Plan Missing)
...
Cohesion: 0.25
Nodes (16): compressWithGD(), compressWithTinyPNG(), handleAddCategory(), handleAddReview(), handleDeleteCategory(), handleDeletePhoto(), handleDeleteReview(), handleEditCategory() (+8 more)
